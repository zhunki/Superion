...
class TokenStreamRewriter(object):
    ...
    def getText(self, program_name):
        ...
        while all((i <= stop, i < len(self.tokens.tokens))):
            ...
            if op is None:
                if token.type != Token.EOF and token.type != 101 and token.type != 102: buf.write(token.text)
                i += 1
            else:
                i = op.execute(buf)
    ...
    def getTextWithoutWhitespace(self, program_name):
        """
        :type interval: Interval.Interval
        :param program_name:
        :param interval:
        ...
        while all((i <= stop, i < len(self.tokens.tokens))):
            token = self.tokens.get(i)
            if token.type != Token.EOF and token.type != 101 and token.type != 102: 
